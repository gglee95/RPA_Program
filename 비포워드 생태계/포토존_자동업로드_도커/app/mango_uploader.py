...
class MangoUploader:
    ...
    async def _dismiss_today_popup(self) -> None:
        """망고카 사이트에 가끔 뜨는 '오늘 하루 보지 않기' 팝업을 닫는다.

        라딕스 UI의 동적 ID (radix-:Rbakva:) 때문에 셀렉터 대신 텍스트로 찾는다.
        팝업이 없으면 조용히 패스 (정상 상태).
        """
        if self.tab is None:
            return
        try:
            ...
            (() => {
                ...
                const target = buttons.find(b => {
                    ...
                    if (!t.includes('오늘 하루 보지 않기')) return false;
                    // 보이는 요소만
            ...
            """)
            if clicked:
                logger.info("팝업 닫기: '오늘 하루 보지 않기' 클릭")
    ...
    async def login(self, email: str, password: str) -> None:
        ...
        # Dismiss "오늘 하루 보지 않기" popup if present (intercepts form clicks otherwise).
        await self._dismiss_today_popup()
