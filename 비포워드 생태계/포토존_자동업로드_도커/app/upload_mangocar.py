...
async def amain() -> int:
    ...
    # Docker 컨테이너에서는 반드시 headless + no_sandbox + 명시적 chromium 경로.
    # CHROME_BIN 환경변수가 있으면 그걸 우선 사용 (Dockerfile에서 /usr/bin/chromium 지정).
    import os
    browser_kwargs: dict = {
        "user_data_dir": str(config.PROFILE_DIR),
        "headless": True,  # 컨테이너는 무조건 headless
        "no_sandbox": True,  # Chromium-in-Docker 필수
    }
    chrome_bin = os.environ.get("CHROME_BIN")
    if chrome_bin:
        browser_kwargs["browser_executable_path"] = chrome_bin
